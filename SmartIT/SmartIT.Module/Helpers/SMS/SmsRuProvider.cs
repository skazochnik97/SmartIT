...
namespace SmartIT.Module.Helpers.SMS
{
    ...
    public class SmsRuProvider : ISmsProvider
    {
        ...
        /*
       ISmsProvider sms = new SmsRuProvider(myComp.SMSUsername, myComp.SMSPassword, myComp.SMSAPIID, myComp.SMSTranslit, true, "c:\\temp\\");
        string code = sms.Send(myComp.SMSFrom, to, text);

        if (code.Substring(0,3) == "100" )
        {
            new GenericMessageBox(e.ShowViewParameters, Application, "Сообщение успешно отправлено!");
        }
        else
        {
            new GenericMessageBox(e.ShowViewParameters, Application, "Сообщение не удалось отправить!");
        }
         */


        public SmsRuProvider(string smsRuLogin, string smsRuPassword, string smsRuApiID, bool translit,bool test,string logfolder)
        {
            this.login = smsRuLogin;
            this.password = smsRuPassword;
            this.apiId = smsRuApiID;
            this.translit = translit;
            this.test = test;
            this.dir = logfolder;

            this.log_sent = logfolder + "Sent.txt";  // "c:\temp\";// = ConfigurationManager.AppSettings["logFolder"] + "Sent.txt";

            log_status = logfolder + "Status.txt";  //= ConfigurationManager.AppSettings["logFolder"] + "Status.txt";
          log_cost = logfolder + "Cost.txt";//= ConfigurationManager.AppSettings["logFolder"] + "Cost.txt";
            log_balance = logfolder + "Balance.txt";// = ConfigurationManager.AppSettings["logFolder"] + "Balance.txt";
            log_limit = logfolder + "Limit.txt"; //= ConfigurationManager.AppSettings["logFolder"] + "Limit.txt";
            log_senders = logfolder + "Senders.txt";//= ConfigurationManager.AppSettings["logFolder"] + "Senders.txt";
            log_auth = logfolder + "AuthCheck.txt";//= ConfigurationManager.AppSettings["logFolder"] + "AuthCheck.txt";
            log_error = logfolder + "Error.txt";//= ConfigurationManager.AppSettings["logFolder"] + "Error.txt";
            log_stoplist = logfolder + "Stoplist.txt"; //= ConfigurationManager.AppSettings["logFolder"] + "Stoplist.txt";
        }
        #region Переменные

        private String login;//= ConfigurationManager.AppSettings["smsRuLogin"];                              // Логин для доступа к SMS.RU
        private String password;// = ConfigurationManager.AppSettings["smsRuPassword"];                        // Пароль для доступа к SMS.RU 
        private String apiId;//= ConfigurationManager.AppSettings["smsRuApiID"];                              // Является вашим секретным кодом, который используется во внешних программах 
        private String partnerId;// = ConfigurationManager.AppSettings["partnerId"];                           // Если вы участвуете в партнерской программе, укажите этот параметр в запросе
        private String SmsRuEmail;// = ConfigurationManager.AppSettings["smsRuApiID"] + "@sms.ru";             // Ваш уникальный адрес (для отправки СМС по email)
        private String email;// = ConfigurationManager.AppSettings["email"];                                   // Ваш email адрес для отправки
        private String smtpLogin;// = ConfigurationManager.AppSettings["smtpLogin"];                           // Логин для авторизации на SMTP-сервере.
        private String smtpPassword;// = ConfigurationManager.AppSettings["smtpPassword"];                     // Пароль для авторизации на SMTP-сервере.
        private String smtpServer; //= ConfigurationManager.AppSettings["smtpServer"];                         // SMTP-сервер.
        private Int32 smtpPort; //= Convert.ToInt32(ConfigurationManager.AppSettings["smtpPort"]);             // Порт для авторизации на SMTP-сервере.
        private Boolean smtpUseSSL;// = Convert.ToBoolean(ConfigurationManager.AppSettings["smtpUseSSL"]);     // Флаг - использовать SSL.
        private Boolean translit; //= Convert.ToBoolean(ConfigurationManager.AppSettings["translit"]);         // Переводит все русские символы в латинские.
        private Boolean test; //= Convert.ToBoolean(ConfigurationManager.AppSettings["test"]);                 // Имитирует отправку сообщения для тестирования ваших программ на правильность обработки ответов сервера. При этом само сообщение не отправляется и баланс не расходуется.
        ...
        private String dir;//= ConfigurationManager.AppSettings["logFolder"];
        private String log_sent;// = ConfigurationManager.AppSettings["logFolder"] + "Sent.txt";
        private String log_status; //= ConfigurationManager.AppSettings["logFolder"] + "Status.txt";
        private String log_cost; //= ConfigurationManager.AppSettings["logFolder"] + "Cost.txt";
        private String log_balance;// = ConfigurationManager.AppSettings["logFolder"] + "Balance.txt";
        private String log_limit; //= ConfigurationManager.AppSettings["logFolder"] + "Limit.txt";
        private String log_senders;//= ConfigurationManager.AppSettings["logFolder"] + "Senders.txt";
        private String log_auth;//= ConfigurationManager.AppSettings["logFolder"] + "AuthCheck.txt";
        private String log_error;//= ConfigurationManager.AppSettings["logFolder"] + "Error.txt";
        private String log_stoplist; //= ConfigurationManager.AppSettings["logFolder"] + "Stoplist.txt";
        #endregion
